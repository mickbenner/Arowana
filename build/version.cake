...
public class BuildVersion
{
    ...
    public static BuildVersion Calculate(ICakeContext context, BuildConfiguration configuration)
    {
        if (context == null)
        {
        ...
        if (!configuration.SkipGitVersion)
        {
            context.Information("Calculating Semantic Version");
            if (!configuration.IsLocalBuild || configuration.IsPublishBuild || configuration.IsReleaseBuild)
            {
                context.GitVersion(new GitVersionSettings{
                    UpdateAssemblyInfoFilePath = "./src/SolutionVersionAssemblyInfo.cs",
    ...
    {
        var solutionInfo = context.ParseAssemblyInfo("./src/SolutionVersionAssemblyInfo.cs");
